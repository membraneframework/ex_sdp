defmodule Membrane.Protocol.SDP.Parser do
  @moduledoc """
  ...
  """
  @spec parse(binary()) ::
          {:ok, SDP.t()} | {:error, atom() | {:not_supported_addr_type, binary()}}
  def parse(binary) do
    binary
  ...
  defp do_parse(lines, session \\ struct(SDP))
  ...
  defp parse_line(["v=" <> version | rest], spec),
    do: {rest, %SDP{spec | version: %Version{value: String.to_integer(version)}}}

  defp parse_line(["o=" <> origin | rest], spec) do
    with {:ok, %Origin{} = origin} <- Origin.parse(origin) do
      {rest, %SDP{spec | origin: origin}}
    end
  end

  defp parse_line(["s=" <> session_name | rest], spec),
    do: {rest, %SDP{spec | session_name: %SessionName{value: session_name}}}

  defp parse_line(["i=" <> session_information | rest], spec),
    do: {rest, %SDP{spec | session_information: %SessionInformation{value: session_information}}}

  defp parse_line(["u=" <> uri | rest], spec),
    do: {rest, %SDP{spec | uri: %URI{value: uri}}}

  defp parse_line(["e=" <> email | rest], spec),
    do: {rest, %SDP{spec | email: %Email{value: email}}}
  ...
  defp parse_line(["m=" <> medium | rest], %SDP{media: media} = session) do
    with {:ok, medium} <- Media.parse(medium),
      ...
      {rest, %SDP{session | media: [medium | media]}}
    end
  end
